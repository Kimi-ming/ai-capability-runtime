...
下一步推荐：先继续当前队列的 T179 Capability graph metadata RFC；如果要优先解除 T205 阻塞，也可以前移 T198 Usage event schema。T070 仍保持 M0 阻塞，解除依赖选择和安装授权后再推进完整 MCP server。
...
当前 next ready task：T179 P2：Capability graph metadata RFC。
