...
本轮继续完成 T283：`packages/runtime/src/ledger.ts` 新增 `FileRuntimeLedgerStore`，实现 `RuntimeLedgerStore` 的本地 JSONL append-only store，默认写入 resolved state dir 下的 `ledger/`，四类 ledger family 分别保存到 `capability.jsonl`、`policy.jsonl`、`invocation.jsonl` 和 `compatibility.jsonl`。新增 `packages/runtime/src/ledger-store.test.ts` 覆盖 append、query、cursor/limit、latest lookup、root export 和 raw payload/secret-shaped metadata 拒绝边界；`@opencap/runtime` root entrypoint 已导出 `FileRuntimeLedgerStore`。下一步进入 T284，把 install/invoke 关键事件写入 Runtime Ledger。
...
`docs/TASKS.md` 已重新拆成 M0-M6 七个模块化任务队列。T070、T198、T205、T282 和 T283 均已完成；当前首个 ready 任务是 T284：把 install/invoke 关键事件写入 Runtime Ledger。

下一步推荐：T284 P1：把 install/invoke 关键事件写入 Runtime Ledger。T291 真实 Claude Desktop/Cursor Host smoke evidence 仍为阻塞任务，只有在用户确认本机 Host 环境可用后再做。
...
- 当前状态：T070、T146、T169、T171、T172、T175、T176、T177、T198、T205、T206、T207、T282 和 T283 已完成并验证；当前 ready 队列从 T284 开始。
