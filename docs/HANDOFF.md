...
Next task: T222 P1：MCP structuredContent adapter。
...
- T222 将把 Result Envelope 适配成 MCP structuredContent/content 输出

## 最近验证

项目 conda 环境 `ai-capability-runtime` 已创建并安装依赖。T221 本轮已运行：
...
T100 已完成。当前 conda 环境下 `pnpm build`、`pnpm test`、`pnpm lint`、`pnpm validate` 均通过；T221 后测试统计为 spec 20 个、cli 1 个、runtime 72 个、mcp 15 个。`pnpm validate` 当前覆盖 5 个 registry manifest 和 5 个 registry test。`node:sqlite` ExperimentalWarning 仍是已知环境提示。

## 下一步建议

1. 实现 T222：MCP structuredContent adapter。
