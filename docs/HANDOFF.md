...
- `@opencap/runtime` 有本地 state dir 初始化、install/list/load installed capabilities、policy parser、Policy Engine、Confirmation Handler、confirmation egress summary、input provenance audit evidence、field-level egress map、derived input evidence chain、input minimization、Data Egress Policy Gate、egress decision audit fields、内存/SQLite Audit Logger、HTTP dry-run plan 和 HTTP executor。
...
Next task: T243 P1：redacted egress preview。
...
- T243 将生成 redacted egress preview，让确认和审计展示外发预览但不暴露 secret/source/internal 原文。

## 最近验证

项目 conda 环境 `ai-capability-runtime` 已创建并安装依赖。T242 本轮已运行：

- `pnpm --filter @opencap/runtime test -- input-minimization.test.ts`
