...
- T218 已记录不同 MCP Host 对 tool metadata 字段的兼容性差异

## 最近验证

项目 conda 环境 `ai-capability-runtime` 已创建并安装依赖。T218 本轮已运行：
...
T100 已完成。当前 conda 环境下 `pnpm build`、`pnpm test`、`pnpm lint`、`pnpm validate` 均通过；T218 后测试统计仍为 spec 20 个、cli 1 个、runtime 63 个、mcp 15 个。`pnpm validate` 当前覆盖 5 个 registry manifest 和 5 个 registry test。`node:sqlite` ExperimentalWarning 仍是已知环境提示。

## 下一步建议

1. 实现 T220：实现 Result Envelope V1。
