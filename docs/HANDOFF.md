...
下一步推荐：`next_task.py` 当前选择 T205 P2：Usage export format。（前置条件已满足：T198 已定义 `opencap.usage_event.v1` schema、必填字段、redaction/non-billing 边界和 audit 关联键）。T205 可基于 T198 的 `opencap.usage_event.v1` schema 定义 JSONL/JSON export envelope、version、filter、redaction 和 compatibility rules。T070 仍保持 M0 阻塞，解除依赖选择和安装授权后再推进完整 MCP server。
...
- 当前状态：T146、T169、T171、T172、T175、T176、T177、T198、T206 和 T207 已完成并验证；T205 已解除阻塞，下一轮可直接定义 usage export format。
...
- T205 P2：Usage export format，直接基于已完成的 T198 Usage event schema。

如果要继续 strict `continuous-doc-dev`，下一轮先处理 T205。

当前 next ready task：T205 P2：Usage export format。
