...
- `@opencap/spec` 有 schema、类型、manifest loader/validator API、registry test 校验、model-visible metadata lint 和 prompt-surface negative fixtures 和 tool projection hash/evidence 和 Runtime-generated risk summary 和 Discovery Profile V1 RFC 和 Selection Evidence record 和 Capability Review model-visible text 检查。
...
Next task: T217 P2：Tool result prompt-surface sanitizer 草案。
...
- T217 将定义 provider response/error 进入模型上下文前的最小 sanitization 策略

## 最近验证

项目 conda 环境 `ai-capability-runtime` 已创建并安装依赖。T216 本轮已运行：
...
T100 已完成。当前 conda 环境下 `pnpm build`、`pnpm test`、`pnpm lint`、`pnpm validate` 均通过；T216 后测试统计仍为 spec 20 个、cli 1 个、runtime 63 个、mcp 15 个。`pnpm validate` 当前覆盖 5 个 registry manifest 和 5 个 registry test。`node:sqlite` ExperimentalWarning 仍是已知环境提示。

## 下一步建议

1. 实现 T217：Tool result prompt-surface sanitizer 草案。
