...
Next task: T214 P2：Discovery profile RFC。
...
- T214 将定义未来 registry discovery/search profile，避免 ranking 或 discovery 污染 policy

## 最近验证

项目 conda 环境 `ai-capability-runtime` 已创建并安装依赖。T213 本轮已运行：
...
T100 已完成。当前 conda 环境下 `pnpm build`、`pnpm test`、`pnpm lint`、`pnpm validate` 均通过；T213 后测试统计为 spec 20 个、cli 1 个、runtime 63 个、mcp 15 个。`pnpm validate` 当前覆盖 5 个 registry manifest 和 5 个 registry test。`node:sqlite` ExperimentalWarning 仍是已知环境提示。

## 下一步建议

1. 实现 T214：Discovery profile RFC。
