...
Next task: T225 P1：Oversized result handling。
...
- T225 将限制 provider JSON/text 大小，避免上下文污染、内存压力和成本失控

## 最近验证

项目 conda 环境 `ai-capability-runtime` 已创建并安装依赖。T224 本轮已运行：
...
T100 已完成。当前 conda 环境下 `pnpm build`、`pnpm test`、`pnpm lint`、`pnpm validate` 均通过；T224 后测试统计为 spec 20 个、cli 1 个、runtime 78 个、mcp 18 个。`pnpm validate` 当前覆盖 5 个 registry manifest 和 5 个 registry test。`node:sqlite` ExperimentalWarning 仍是已知环境提示。

## 下一步建议

1. 实现 T225：Oversized result handling。
