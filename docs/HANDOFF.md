...
Next task: T224 P1：Result provenance/evidence。
...
- T224 将记录 result provenance、content digest、transformations 和 taint labels

## 最近验证

项目 conda 环境 `ai-capability-runtime` 已创建并安装依赖。T223 本轮已运行：
...
T100 已完成。当前 conda 环境下 `pnpm build`、`pnpm test`、`pnpm lint`、`pnpm validate` 均通过；T223 后测试统计为 spec 20 个、cli 1 个、runtime 76 个、mcp 18 个。`pnpm validate` 当前覆盖 5 个 registry manifest 和 5 个 registry test。`node:sqlite` ExperimentalWarning 仍是已知环境提示。

## 下一步建议

1. 实现 T224：Result provenance/evidence。
