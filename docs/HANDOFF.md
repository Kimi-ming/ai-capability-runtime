...
本轮完成了项目完成度审查和自主开发推进：T124 Runtime 领域模型 public contract、T145 workspace package public exports、T268 Runtime Gate public contract、T269 Runtime Ledger storage contract、T270 Runtime Card schema contract、T273 Capability identity contract、T275 Capability authoring loop、T276 release maturity gate matrix、T131 `execution.body.fields` 渲染边界、T132 audit failure preflight、T133 outbound policy 私网阻断、T135 state dir precedence tests、T152 consent receipt audit fields 和 T160 credential descriptor schema tests 均已落入代码或文档并验证。当前唯一明确阻塞项仍是 T070：选择并接入 MCP TypeScript SDK，需要确认依赖包名/版本并允许安装新 npm 依赖。
...
- Capability authoring loop/lint 顺序 contract，且 `pnpm validate` 已把 model-visible metadata lint 和 credential descriptor schema 纳入 registry validation
...
- `@opencap/spec` 有 schema、类型、manifest loader/validator API、Capability authoring loop contract、registry test 校验、schema JSON public subpath export、`api_key` auth placement 和 credential descriptor 约束测试、model-visible metadata lint、authoring manifest validation 和 prompt-surface negative fixtures。
...
`docs/TASKS.md` 已重新拆成 M0-M6 七个模块化任务队列。`next_task.py` 现在能识别开放任务，T160 已完成；下一项预计进入 M2 / T161：实现 least-privilege auth lint。

下一步推荐：先在 M2 模块内补齐 T161 的验收标准和任务级验证，再按 TDD 进入实现。T070 仍保持 M0 阻塞，解除依赖选择和安装授权后再推进完整 MCP server。

推荐第一个任务：

```text
M2 / T161 P1：实现 least-privilege auth lint
```

原因：

-  已完成
...
- 当前状态：T160 已完成并验证；下一轮从 `next_task.py` 重新选择，预计进入 T161。
...
当前已知：上述验证均通过。`pnpm test` 当前覆盖 spec 37 个、runtime 208 个、mcp 18 个、cli smoke 1 个测试。运行环境使用 conda `ai-capability-runtime` 中的 Node 22 和 pnpm 9.15.3。`node:sqlite` ExperimentalWarning 仍是已知环境提示。Runtime HTTP executor 测试需要绑定 `127.0.0.1`；在 Codex 沙箱内会因 `listen EPERM` 失败，本轮已脱离沙箱重跑通过。
...
如果要继续 strict `continuous-doc-dev`，下一轮先把 T161 展开为带验收标准和验证命令的任务块，再按 TDD 实现。
