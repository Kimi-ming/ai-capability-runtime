...
- `@opencap/spec` 有 schema、类型、manifest loader/validator API、Capability authoring loop contract、registry test 校验、schema JSON public subpath export、`api_key` auth placement 约束测试、model-visible metadata lint、authoring manifest validation 和 prompt-surface negative fixtures。
...
- 当前状态：T132 已完成并验证；下一轮从 `next_task.py` 重新选择，预计进入 T133。
...
当前已知：上述验证均通过。`pnpm test` 当前覆盖 spec 32 个、runtime 191 个、mcp 18 个、cli smoke 1 个测试。运行环境使用 conda `ai-capability-runtime` 中的 Node 22 和 pnpm 9.15.3。`node:sqlite` ExperimentalWarning 仍是已知环境提示。Runtime HTTP executor 测试需要绑定 `127.0.0.1`；在 Codex 沙箱内会因 `listen EPERM` 失败，本轮已脱离沙箱重跑通过。
...
如果要继续 strict `continuous-doc-dev`，下一轮先把 T133 展开为带验收标准和验证命令的任务块，再按 TDD 实现。
