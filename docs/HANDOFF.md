...
- T221 将实现 output schema validation，确保 structured output 校验后才能 success

## 最近验证

项目 conda 环境 `ai-capability-runtime` 已创建并安装依赖。T220 本轮已运行：
...
T100 已完成。当前 conda 环境下 `pnpm build`、`pnpm test`、`pnpm lint`、`pnpm validate` 均通过；T220 后测试统计为 spec 20 个、cli 1 个、runtime 68 个、mcp 15 个。`pnpm validate` 当前覆盖 5 个 registry manifest 和 5 个 registry test。`node:sqlite` ExperimentalWarning 仍是已知环境提示。

## 下一步建议

1. 实现 T221：实现 output schema validation。
