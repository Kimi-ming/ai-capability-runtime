...
- T235 将补充可复用 sensitive input classification fixtures，覆盖 secret-like、pii、internal URL、source/config 和 large free text unknown。

## 最近验证

项目 conda 环境 `ai-capability-runtime` 已创建并安装依赖。T234 本轮已运行：
...
T100 已完成。当前 conda 环境下 `pnpm build`、`pnpm test`、`pnpm lint`、`pnpm validate` 均通过；T234 后测试统计为 spec 20 个、cli 1 个、runtime 91 个、mcp 18 个。`pnpm validate` 当前覆盖 5 个 registry manifest 和 5 个 registry test。`node:sqlite` ExperimentalWarning 仍是已知环境提示。
...
1. 实现 T235：补充 sensitive input classification fixtures。
