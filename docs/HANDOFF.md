...
Next task: T215 P2：Selection evidence record。
...
- T215 将定义 Host/model 选择工具时可选记录的 selection evidence 字段

## 最近验证

项目 conda 环境 `ai-capability-runtime` 已创建并安装依赖。T214 本轮已运行：
...
T100 已完成。当前 conda 环境下 `pnpm build`、`pnpm test`、`pnpm lint`、`pnpm validate` 均通过；T214 后测试统计仍为 spec 20 个、cli 1 个、runtime 63 个、mcp 15 个。`pnpm validate` 当前覆盖 5 个 registry manifest 和 5 个 registry test。`node:sqlite` ExperimentalWarning 仍是已知环境提示。

## 下一步建议

1. 实现 T215：Selection evidence record。
