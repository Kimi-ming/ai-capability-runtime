...
- `@opencap/runtime` 有本地 state dir 初始化、install/list/load installed capabilities、policy parser、Policy Engine、Confirmation Handler、Data Egress Policy Gate、内存/SQLite Audit Logger、HTTP dry-run plan 和 HTTP executor。
...
- T237 将把 Data Egress Policy Gate 的 decision、data classes、egress target、matched rule 和 redacted preview 写入审计证据。

## 最近验证

项目 conda 环境 `ai-capability-runtime` 已创建并安装依赖。T236 本轮已运行：
