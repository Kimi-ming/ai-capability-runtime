...
- 如果更适合从 TypeScript 生成草稿，可以用 `@opencap/sdk` 的 `defineHttpCapabilityManifest()` / `defineValidHttpCapabilityManifest()` 定义并校验 V1 HTTP manifest draft。SDK helper 只复用 authoring validation 返回 manifest 或 issues，不是 Runtime plugin API，不接受 `run()` handler，不安装、不执行、不读取 secret、不触网，也不改变 policy、audit 或 trust；输出仍必须进入 `opencap validate`、package lint、model-visible metadata lint、least-privilege/risk lint、registry tests 和 review。
...
- 用 `opencap registry index build --registry registry --output <file>` 保存 `opencap.registry.index.v1` / `opencap.registry.index_cache_sync.v1` 本地发现索引，再用 `opencap registry index validate --file <file> --output <validation-report>` 保存 `opencap.registry_index_validation.v1` validation report。该 unsigned artifact 和 validation report 只是 discovery/cache evidence，validation 只证明 saved index artifact 的结构、脱敏边界和 digest 一致性；它们不替代 manifest validation、Registry review、package lint、advisory/lifecycle gates、本地 policy、release evidence validation、签名验证或 install/execute 审计，也不包含 manifest 原文、auth env、secret、provider raw response、input/output 原文、`opencap.local/`、DB/log 或私有绝对路径。
- 准备 release/conformance evidence 时，可以用 `opencap registry report --registry registry --output <file>` 保存 `opencap.registry_quality_summary.v1`，并用 `opencap conformance report --records packages/runtime/test/fixtures/conformance --output <file>` 保存 `opencap.conformance_summary.v1`。这两类 artifact 只是本地 evidence，不替代 GitHub required checks、真实 Host UI smoke、release approval、tag 创建或 npm 发布。
