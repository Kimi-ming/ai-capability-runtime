...
- 用 `opencap registry index build --registry registry --output <file>` 保存 `opencap.registry.index.v1` / `opencap.registry.index_cache_sync.v1` 本地发现索引；该 unsigned artifact 只是 discovery/cache input，不替代 manifest validation、Registry review、package lint、advisory/lifecycle gates、本地 policy、release evidence validation、签名验证或 install/execute 审计，也不包含 manifest 原文、auth env、secret、provider raw response、input/output 原文、`opencap.local/`、DB/log 或私有绝对路径。
