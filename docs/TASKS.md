...
- [x] T319 P1：增强 release artifact decision consistency 校验。
  - 验收标准：
    ...
    - 如果 `decision: release` 但存在任何 `not-run` command 或 `not-run` registry/conformance component，validation report 必须返回 invalid finding，避免把 incomplete local evidence 写成 release-ready。
    - Finding 不包含 secret、provider raw response、input/output 原文、`opencap.local/` 内容、数据库日志或私有路径。
  - 验证方式：
    - `pnpm --filter @opencap/spec test -- release-evidence-artifact.test.ts`
    - `pnpm --filter @opencap/spec build`
    - `pnpm validate`
...
- [x] T320 P1：支持 release artifact validation report 安全写入文件。
  - 验收标准：
    ...
    - 对 invalid artifact，命令仍写出脱敏 validation report 并以 exit `1` 结束；对非法 JSON artifact，不写 validation report。
    - 非法 `--output` 作为用户错误 exit `1`，stderr 不打印 stack，且不留下半截 report 文件。
  - 验证方式：
    - `pnpm --filter @opencap/cli test -- release-artifact-command.test.ts`
    - `pnpm --filter @opencap/cli build`
    - `pnpm validate`
...
- [x] T321 P2：把 validation report artifact 纳入发布文档。
  - 验收标准：
    ...
    - `docs/TESTING.md` 记录 `opencap release artifact validate --output` 命令测试入口。
    - Handoff 指向下一项 ready 或明确剩余外部限制。
  - 验证方式：
    - `python3 /Users/kimi/.codex/skills/continuous-doc-dev/scripts/check_docs.py .`
    - `python3 /Users/kimi/.codex/skills/continuous-doc-dev/scripts/audit_docs.py .`
...
- [x] T330 P2：把 advisory check/list 纳入安全和运维文档。
  - 验收标准：
  ...
  - 验证方式：
    ...
    - `python3 /Users/kimi/.codex/skills/continuous-doc-dev/scripts/audit_docs.py .`
    - `git diff --check`
  - 完成记录：README 快速开始和中文文档中心已加入 `opencap registry advisory list --registry registry --json` 与 `opencap advisory check --state-dir <path> --registry registry --json` 入口；`docs/安全/capability-advisory-process.md`、`docs/生态/capability-deprecation-and-revocation.md`、`docs/社区/registry-guidelines.md` 和 `registry/README.md` 已明确 advisory check/list 只产生本地 evidence/告警，不自动卸载、不自动授权、不自动修改 policy，也不读取 secret、调用 provider 或触网。`docs/TESTING.md` 已记录两个 CLI 测试入口、CLI 64 个测试和不触网/不读取 secret 边界。Handoff 已指向下一批 ready 任务 T331-T333。

- [x] T331 P2：新增 `opencap registry advisory show` 本地公告详情命令。
  - 验收标准：
    - CLI `opencap registry advisory show <advisory-id> --registry <path> [--json]` 精确读取本地 Registry advisory metadata 并输出单条 Capability Advisory detail。
    - JSON 输出使用稳定 schema `opencap.registry_advisory_detail.v1`，包含 advisory id、capability、affectedVersions、type、severity、status、summary、publishedAt、modifiedAt、registryAction、runtimeDefault、fixedVersion、references、filePath 和 `policyEffect: none`。
    - 人类输出包含同等审查摘要；not found、重复 id、invalid advisory 文件作为用户错误 exit `1` 且不打印 stack。
    - 命令不安装、不写 state dir、不读取 secret、不调用 provider、不触网。
  - 验证方式：
...
- [x] T336 P2：把 advisory check filters 纳入安全和运维文档。
  - 验收标准：
...
- [x] T337 P2：为 `opencap advisory check` 增加安全 JSON evidence 输出文件。
  - 验收标准：
    ...
    - 输出路径必须拒绝 `.env`、token/secret/password 命名、`opencap.local/`、SQLite/DB/log 文件和目录路径；失败时不留下半截文件。
    - revoked/revoke/deny matches 和 invalid advisory summary 仍保持既有 exit `1` 语义；输出 report 不执行 Capability、不写 audit logs、不读取 provider secret、不调用 provider、不触网。
  - 验证方式：
    - `pnpm --filter @opencap/cli test -- advisory-check-command.test.ts`
    - `pnpm --filter @opencap/cli build`
    - `pnpm validate`
...
- [x] T338 P2：增强 `opencap advisory check` 人类摘要的 filters 和 match counts。
  - 验收标准：
    ...
    - 人类摘要不输出 input/output、secret、Authorization/Cookie、provider raw response 或本地数据库日志路径。
  - 验证方式：
    - `pnpm --filter @opencap/cli test -- advisory-check-command.test.ts`
    - `pnpm --filter @opencap/cli build`
    - `pnpm validate`
...
- [x] T340 P2：为 `opencap registry advisory list` 增加安全 JSON evidence 输出文件。
  - 验收标准：
...
- [x] T343 P2：为 `opencap registry report` 增加安全 JSON evidence 输出文件。
  - 验收标准：
