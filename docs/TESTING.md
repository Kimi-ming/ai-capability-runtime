...
- `packages/cli/src/smoke.test.ts`：CLI 端到端 smoke，使用临时 `--state-dir` 跑通 validate、install、list、invoke dry-run Result Envelope JSON、dry-run egress preview、人类 summary 输出、secret missing verbose redacted evidence 和 logs，当前覆盖 1 个测试。
