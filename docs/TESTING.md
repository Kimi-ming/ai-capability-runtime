...
该命令只从本地 SQLite audit log 的脱敏 operational metadata 派生 summary；它不读取 provider secret，不执行 Capability，不输出 input/output/egress preview 原文。
