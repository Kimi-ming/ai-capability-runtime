...
最近全量验证：2026-05-13 在项目 conda 环境中通过 `pnpm validate`、`pnpm lint`、`pnpm build` 和 `pnpm test`。当前 `pnpm validate` 覆盖 5 个 registry manifest、5 个 manifest 的 model-visible metadata lint 和 5 个 registry test；`pnpm test` 覆盖 spec 32 个、runtime 191 个、mcp 18 个、cli smoke 1 个测试。`node:sqlite` ExperimentalWarning 仍是已知环境提示。
...
- `packages/cli/src/smoke.test.ts`：CLI 端到端 smoke，使用临时 `--state-dir` 跑通 validate、install、list、invoke dry-run Result Envelope JSON、dry-run egress preview、人类 summary 输出、policy explain、policy validate lint、policy simulation/diff、secret missing verbose redacted evidence、logs 和 decision-log export，当前覆盖 1 个测试。
...
- Secret Resolver env provider、credential audit evidence 和 audit failure preflight 已实现；outbound policy 私网阻断和更完整的 conformance smoke 仍是后续任务，需要按下面的模块计划继续补齐。
...
该 job 不需要外部 secret，失败时应优先检查 Capability manifest schema、model-visible metadata lint 和 `tests/basic.yml` registry test 格式。workflow 也保留 workspace tests job，用于运行：
...
当前 `pnpm validate` 已能校验 registry manifests、model-visible metadata lint 和 registry tests，`packages/spec/src/index.test.ts` 已覆盖 validator 基础行为和 `auth.placement` 约束，`packages/spec/src/metadata-lint.test.ts` 已覆盖模型可见 metadata lint，`packages/spec/src/authoring.test.ts` 已覆盖 authoring loop/lint 顺序。后续继续扩展时应保持覆盖：
