...
CONFORMANCE_OUTPUT="$(mktemp "${TMPDIR:-/tmp}/opencap-conformance-summary.XXXXXX")"
pnpm --filter @opencap/cli dev -- conformance report --records packages/runtime/test/fixtures/conformance --output "$CONFORMANCE_OUTPUT"
```
...
pnpm --filter @opencap/cli dev -- registry index validate --file "$REGISTRY_INDEX_OUTPUT" --json
REGISTRY_INDEX_VALIDATION_OUTPUT="$(mktemp "${TMPDIR:-/tmp}/opencap-registry-index-validation.XXXXXX")"
pnpm --filter @opencap/cli dev -- registry index validate --file "$REGISTRY_INDEX_OUTPUT" --output "$REGISTRY_INDEX_VALIDATION_OUTPUT"
```

`registry index build` 从本地 Registry checkout 生成 `opencap.registry.index.v1` / `opencap.registry.index_cache_sync.v1` discovery/cache artifact；`--output` 会安全写出 JSON，并拒绝 `.env`、token/secret/password 命名、`opencap.local/`、SQLite/DB/log 和目录路径。`registry index validate` 校验保存后的 unsigned index artifact，输出 `opencap.registry_index_validation.v1` report；JSON/人类输出都保持脱敏，invalid artifact 返回 exit `1`，`--output` 会安全写出 validation report JSON，非法 JSON 或读取失败不会写半截 report。对应测试入口是 `packages/spec/src/registry-index.test.ts`、`packages/spec/src/registry-index-artifact.test.ts`、`packages/cli/src/registry-index-command.test.ts` 和 `packages/cli/src/registry-index-validate-command.test.ts`，覆盖 schema/profile、相对 manifest path、manifest digest、quality/advisory/trust 摘要、`signatureStatus: none`、`policyEffect: none`、稳定 `indexDigest`、保存后 artifact validation、JSON/人类输出、安全 output、危险 output 用户错误和非法 JSON 不写 partial report。该 unsigned artifact 和 validation report 只是本地发现和 cache/sync evidence，不替代 manifest validation、Registry review、package lint、advisory/lifecycle gates、本地 policy、release evidence validation、签名验证或 install/execute 审计；输出不包含 manifest 原文、auth env、secret、provider raw response、input/output/execution 原文、`opencap.local/`、DB/log 或私有绝对路径。命令不安装或执行 Capability、不写 state dir、不读取 provider secret、不触网，也不改变 trust、policy、authorization 或 Runtime execution。
...
`opencap release package report` 只汇总本地 package metadata 和可选 `npm pack --dry-run --json` 摘要；`--output` 会安全写出 `opencap.npm_package_readiness.v1` JSON evidence report，并拒绝 `.env`、token/secret/password 命名、`opencap.local/`、SQLite/DB/log 和目录路径。随后 `opencap release evidence --package-readiness <file>` 可复用并校验一个或多个 artifacts，重复参数会按传入顺序纳入 `opencap.release_evidence.v1` bundle；这些输入不能与 `--package`/`--pack-json` 混用。对应测试入口是 `packages/cli/src/release-package-report-command.test.ts` 和 `packages/cli/src/release-evidence-command.test.ts`，当前覆盖 JSON、人类输出、`--pack-json` dry-run 摘要、`--output` 安全 JSON evidence 写入、`--package-readiness` artifact 输入、重复 `--package-readiness` 多 artifact 输入、invalid artifact 不写 output、unsupported package / invalid pack JSON 不写 output、非法 output 用户错误和本地 npm pack dry-run smoke。该 artifact 只证明本地 package metadata/tarball 摘要审查，不替代 `npm pack --dry-run --json` 原始摘要、workflow publish dry-run、trusted publisher、正式 provenance、release approval 或 npm 发布；命令不触网、不读取 npm token、不写 state dir，也不改变 package publish state、trust、policy、authorization 或 Runtime execution。
...
REGISTRY_ADVISORY_LIST_OUTPUT="$(mktemp "${TMPDIR:-/tmp}/opencap-registry-advisory-list.XXXXXX")"
pnpm --filter @opencap/cli dev -- registry advisory list --registry registry --output "$REGISTRY_ADVISORY_LIST_OUTPUT"
pnpm --filter @opencap/cli dev -- registry advisory show OCAP-2026-0001 --registry registry --json
REGISTRY_ADVISORY_DETAIL_OUTPUT="$(mktemp "${TMPDIR:-/tmp}/opencap-registry-advisory-detail.XXXXXX")"
pnpm --filter @opencap/cli dev -- registry advisory show OCAP-2026-0001 --registry registry --output "$REGISTRY_ADVISORY_DETAIL_OUTPUT"
...
- `packages/sdk-js/src/index.test.ts`：SDK V1 manifest authoring helper，覆盖 valid HTTP Capability Manifest draft、invalid manifest issue propagation、strict helper 的 `SdkCapabilityAuthoringError`、拒绝 runtime `run()` handler 和 `SDK_AUTHORING_BOUNDARY` root export 边界；SDK 包合计 4 个测试。该 helper 只复用 `@opencap/spec` authoring validation 定义/校验 manifest draft，不安装或执行 Capability、不读取 provider secret、不写 state dir、不触网，也不改变 trust、policy、authorization 或 Runtime execution。
- `packages/cli/src/init-command.test.ts`：CLI init scaffold tests，覆盖 `opencap init <capability-id> --category <name> --output <dir>` 写出 `manifest.yml`、`README.md` 和 `tests/basic.yml`、生成 package 可被 `opencap validate` 验证、不写 `opencap.local`、覆盖保护、危险 output 拒绝和非法 id 用户错误无 stack，当前覆盖 4 个测试。`packages/cli/src/registry-search-command.test.ts`：CLI registry search tests，覆盖 `opencap registry search [query] --registry <path> --json` 的 `opencap.registry_search.v1` report、人类输出、默认隐藏 yanked/revoked、`--include-lifecycle yanked,revoked` 显式纳入、invalid manifest exit `1`、非法 include lifecycle 用户错误和不写 state dir 边界，当前覆盖 4 个测试。`packages/cli/src/registry-show-command.test.ts`：CLI registry show tests，覆盖 `opencap registry show <capability-id> --registry <path> --json` 的 `opencap.registry_capability_detail.v1` report、人类输出、auth/permission/execution 摘要、默认隐藏 revoked、显式 include lifecycle、未找到、多重匹配、invalid manifest、非法 include lifecycle 用户错误和不写 state dir 边界，当前覆盖 4 个测试。`packages/cli/src/advisory-check-command.test.ts`：CLI advisory check tests，覆盖 `opencap advisory check --state-dir <path> --registry <path> --json` 的 `opencap.advisory_check.v1` report、`--capability` installed capability filter、`--severity`/`--status` match filters、`--output` 安全 JSON evidence 写入、raw/filtered match counts、人类摘要 filters/match counts、非法 filter/output 用户错误、revoked/deny advisory exit `1`、warning-only advisory exit `0`、空安装状态人类输出、invalid advisory JSON summary 和不写 audit logs/不读取 secret 边界，当前覆盖 10 个测试。`packages/cli/src/registry-advisory-list-command.test.ts`：CLI registry advisory list tests，覆盖 `opencap registry advisory list --registry <path> --json` 的 `opencap.registry_advisory_list.v1` report、人类输出、空 advisory 友好提示、`--capability`/`--severity`/`--status` filters、`--output` 安全 JSON evidence 写入、invalid advisory JSON summary 写入、非法 filter/output 用户错误、exit `1` 和不写 state dir/不读取 secret 边界，当前覆盖 7 个测试。`packages/cli/src/registry-advisory-show-command.test.ts`：CLI registry advisory show tests，覆盖 `opencap registry advisory show <advisory-id> --registry <path> --json` 的 `opencap.registry_advisory_detail.v1` report、人类输出、`--output` 安全 JSON evidence 写入、not found、重复 advisory id、invalid advisory 不写 output、非法 output 用户错误和不写 state dir/不读取 secret 边界，当前覆盖 5 个测试。`packages/cli/src/metrics-command.test.ts`：CLI metrics tests，覆盖 `opencap metrics summary --state-dir <path> --json` 的脱敏 summary、人类输出、capability filter、secret missing 计数、非法 since/until 用户错误和参数校验早于 SQLite 初始化，并覆盖 `opencap metrics capabilities --json` 的 per-capability aggregate、error rate、last seen 和 `opencap metrics security` 的 JSON/人类输出，当前覆盖 5 个测试。`packages/cli/src/release-evidence-command.test.ts`：CLI release evidence tests，覆盖 `opencap release evidence --registry <path> --records <path> --json` 的脱敏 bundle、默认非 epoch 时间戳、`--generated-at` 可复现输入、`--output` 安全写文件、危险输出路径拒绝、人类输出时间字段、inline package readiness 输入、`--package-readiness` artifact 输入、重复 `--package-readiness` 多 artifact 输入顺序、invalid artifact 不写 output、非法时间参数和 package 参数用户错误，当前覆盖 11 个测试。`packages/cli/src/release-artifact-command.test.ts`：CLI release artifact validation tests，覆盖 `opencap release artifact validate --file <path> --json` 的 valid report、人类输出、含 secret/path/log artifact 的 invalid report、`--output` 安全写 validation report、invalid artifact 写 report 后 exit `1`、非法 JSON 不写 report 和非法 JSON 用户错误，当前覆盖 8 个测试。`packages/cli/src/release-package-report-command.test.ts`：CLI release package report tests，覆盖 `opencap release package report --package <name> --json` 的 package readiness report、`--pack-json` dry-run 摘要、`--output` 安全 JSON evidence 写入、forbidden files、人类输出、unsupported package 和 invalid pack JSON 不写 output、非法 output 用户错误，并使用临时 npm cache 对 `@opencap/spec` / `@opencap/cli` 跑本地 `npm pack --dry-run --json` smoke，当前覆盖 7 个测试。`packages/cli/src/conformance-report-command.test.ts`：CLI conformance report tests，覆盖 `opencap conformance report --records <path> --json` 的脱敏 summary、`--output` 安全 JSON evidence 写入、invalid summary output 脱敏、非法 output 用户错误、临时 cwd 不创建 `opencap.local`、人类输出和 invalid record exit `1`，当前覆盖 6 个测试。`packages/cli/src/doctor-command.test.ts`：CLI doctor JSON tests，覆盖空 state 的脱敏 machine-readable diagnostics、已安装 state、invalid installed entry 和人类输出兼容，当前覆盖 2 个测试。`packages/cli/src/ledger-command.test.ts`：CLI Runtime Ledger export tests，覆盖空 ledger 友好输出、JSON 导出、kind/capability/limit 筛选、secret-missing blocked invocation 脱敏和非法参数 exit `1`，当前覆盖 3 个测试。`packages/cli/src/registry-report-command.test.ts`：CLI Registry report tests，覆盖 `opencap registry report --registry <path> --json` 的脱敏 quality summary、`--output` 安全 JSON evidence 写入、非法 output 用户错误、临时 cwd 不创建 `opencap.local`，以及人类输出的 lifecycle/advisory/quality/blocking summary，当前覆盖 4 个测试。`packages/cli/src/registry-index-command.test.ts`：CLI Registry index build tests，覆盖 `opencap registry index build --registry <path> --json` 的脱敏 discovery/cache index artifact、人类摘要、安全 `--output` JSON artifact、危险 output 用户错误、临时 cwd 不创建 `opencap.local`、manifest path 相对化和 raw manifest/input/output/execution/secret/path 脱敏边界，当前覆盖 4 个测试。`packages/cli/src/registry-index-validate-command.test.ts`：CLI Registry index validate tests，覆盖 `opencap registry index validate --file <path> --json` 的 validation report、人类摘要、invalid artifact 安全 output、危险 output 用户错误、非法 JSON 不写 partial report 和临时 cwd 不创建 `opencap.local`，当前覆盖 4 个测试。`packages/cli/src/card-command.test.ts`：CLI Capability/Trust Card output tests，覆盖已安装 Capability 输出脱敏 `opencap.card.v1` JSON、manifest digest evidence、auth credential redaction、`--kind trust` 输出 trust level/maintainer/advisory/provenance/limitations/disclaimer、Trust Card 不修改 state，以及未安装和非法 `--kind` 返回用户错误 exit `1` 且不打印 stack，当前覆盖 4 个测试。`packages/cli/src/error-exit-code.test.ts`：CLI error model / exit code tests，覆盖 missing installed capability、invalid `--input-json` 和 invalid `--limit` 作为用户可修正错误返回 exit `1`，stderr 不输出 stack trace，当前覆盖 2 个测试。`packages/cli/src/command-snapshot.test.ts`：CLI command snapshot tests，使用临时 `--state-dir` 覆盖 validate 成功输出、空 list/logs/decision-log、已安装 list 人类表格和 JSON 输出、policy validate 用户错误输出与 exit code，并归一化 `<repo>`、`<state>` 和 Node warning pid，当前覆盖 2 个测试。`packages/cli/src/smoke.test.ts`：CLI 端到端 smoke，使用临时 `--state-dir` 跑通 validate、install、list JSON 和人类表格 lifecycle/trust 字段、invoke dry-run Result Envelope JSON、dry-run egress preview、人类 summary 输出、policy explain、policy validate lint、policy simulation/diff、secret missing verbose redacted evidence、install/invoke Runtime Ledger JSONL 写入、logs 和 decision-log export，当前覆盖 1 个测试；CLI 包合计 97 个测试。
