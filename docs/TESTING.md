...
对应测试入口是 `packages/cli/src/registry-advisory-list-command.test.ts`、`packages/cli/src/registry-advisory-show-command.test.ts` 和 `packages/cli/src/advisory-check-command.test.ts`；当前合计覆盖 10 个 CLI 测试，CLI 包总数为 67 个测试。这些命令只读取本地 Registry advisory metadata 和本地 installed capability metadata，不执行 Capability、不读取 provider secret、不调用 provider、不触网、不自动卸载、不自动授权、不自动修改 policy。
