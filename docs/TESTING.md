...
最近全量验证：2026-05-12 在项目 conda 环境中通过 `pnpm validate`、`pnpm lint`、`pnpm build` 和 `pnpm test`。当前 `pnpm validate` 覆盖 5 个 registry manifest 和 5 个 registry test；`pnpm test` 覆盖 spec 25 个、runtime 161 个、mcp 18 个、cli smoke 1 个测试。`node:sqlite` ExperimentalWarning 仍是已知环境提示。
...
当前 `pnpm validate` 已能校验 registry manifests，`packages/spec/src/index.test.ts` 已覆盖 validator 基础行为和 `auth.placement` 约束，`packages/spec/src/metadata-lint.test.ts` 已覆盖模型可见 metadata lint。后续继续扩展时应保持覆盖：
