...
- `@opencap/spec` 新增 OpenAPI operation selection evidence helper，输出本地 `opencap.openapi_operation_selection.v1` report，校验显式 method/path 选择、HTTPS server、cookie/unsupported security scheme、deprecated/schema review warning、reviewer hints 和敏感文本脱敏边界，固定 `policyEffect: none`。
